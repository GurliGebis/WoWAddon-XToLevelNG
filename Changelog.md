# 12.0.1-20260211-1
* Updated TOC to Midnight Pre-Patch.

# 12.0.0-20260209-1
* Added TOC for The Burning Crusade Anniversary and for Classic.
* Make sure unit exists before checking if it is secret.

# 12.0.0-20260126-1
* Renable COMBAT_LOG_EVENT_UNFILTERED for classic and use PARTY_KILL in retail.
* Tooltip fixes for secret values.

# 12.0.0-20260121-2
* Disable COMBAT_LOG_EVENT_UNFILTERED event, until workaround can be found.

# 12.0.0-20260121-1
* Updated TOC to Midnight Pre-Patch.

# 11.2.7-20251210-1
* Updated TOC for Mop Classic.

# 11.2.7-20251204-1
* Updated TOC for Mop Classic.

# 11.2.7-20251202-1
* Updated TOC to match WoW 11.2.7

# 11.2.5-20251006-1
* Updated TOC to match WoW 11.2.5

# 11.2.0-20250924-1
* Updated Classic TOC to match 5.5.1

# 11.2.0-20250806-1
* Updated TOC to match WoW 11.2.0

# 11.1.7-20250619-1
* Updated TOC to match WoW 11.1.7

# 11.1.5-20250610-1
* Added support for Classic and MoP Classic.

# 11.1.5-20250422-1
* Updated TOC to match WoW 11.1.5

# 11.1.0-20250225-1
* Updated TOC to match WoW 11.1.0

# 11.0.7-20250128-1
* Added category to TOC file.

# 11.0.7-20241218-1
* Updated TOC to match WoW 11.0.7

# 11.0.5-20241023-1
* Updated TOC to 11.0.5

# 11.0.2-20241015-1
* Added installation of missing github runner packages.

# 11.0.2-20240912-1
* Github actions fixes.

# 11.0.2-20240814-1
* Changes needed due to deprecated API calls.

# 11.0.2-20240813-1
* Updated TOC to 11.0.2

# 11.0.0-20240725-3
* Fix logic to open settings dialog.

# 11.0.0-20240725-2
* Updated localization to be embedded instead of downloaded from Curseforge.
* Change so libraries are downloaded instead of embedded from github.

# 11.0.0-20240725-1
* Support for The War Within.

# 10.2.7-20240508_1
* Updated TOC to 10.2.7

# 10.2.6-20240320-1
* Updated TOC to 10.2.6

# 10.2.5-20240117_1
* Updated TOC to 10.2.5

# 10.2.0-20231207_1
* Handle if GetAverageBGObjectiveXP returns nil.

# 10.2.0-20231108_1
* Updated TOC to 10.2.0

# 10.1.7-20230912_1
* Updated TOC to 10.1.7

# 10.1.5-20230712_1
* Updated TOC to 10.1.5

# 10.1.0-20230503_1
* Updated TOC to 10.1.0

# 10.0.7-20230322_1
* Updated TOC to 10.0.7

# 10.0.5-20230220_1
* Added support for Wotlk classic.

# 10.0.5-20230125_2
* Forgot to change compatibility version.

# 10.0.5-20230125_1
* Updated TOC to 10.0.5

# 10.0.2-20221129_1
* First commit, support for 10.0.2
